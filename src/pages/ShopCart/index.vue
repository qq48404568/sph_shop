...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      // type：为了区分这三个元素
      // 目前disNum形参:+变化量(1) -变化量(-1)  input 最终的个数（并不是变化量）
      // cart：哪一个产品【身上有id】
      // console.log('派发actions',type,disNum,cart);
      switch (type) {
        case 'add':
          disNum = 1
          break;
    ...
    async updateChecked (cart, event) {
      try {
        ...
        await this.$store.dispatch('deleteCartListBySkuId', { skuId: cart.skuId, isChecked: isChecked })
        this.getData()
      } catch (error) {
        alert(error.message)
      }
